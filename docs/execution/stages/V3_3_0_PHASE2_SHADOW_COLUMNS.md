...
1. 在所有 30 个登记金额字段旁新增统一的整数分影子列；列名、空值和符号规则由金额注册表生成或校验。
2. 新增金额迁移状态表、批次表和异常表，记录版本、范围、游标、摘要、错误和重跑状态。
3. 编写可重复回填：历史 INT 元乘 100；DECIMAL 元按精确十进制转分；重复执行不改变已一致数据。
4. 生成逐表数量、合计、最小/最大值、负数和不一致摘要，任何不可解释数据进入异常表，不静默修复。
5. 完成空库迁移、V3.2.4 数据升级、部分失败重跑和数据库备份恢复演练。

## 3. 明确不做

- 不修改旧列语义；不让业务读写影子列；不开放小数输入；不删除旧列。
## 统一执行要求
...
- 影子列和旧列逐行可证明等价；回填可重复；异常为零或全部有明确处置；迁移和回滚演练通过。
