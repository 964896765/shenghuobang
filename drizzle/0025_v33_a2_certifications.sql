...
ALTER TABLE `certification_documents` ADD CONSTRAINT `certification_documents_run_fk` FOREIGN KEY (`migrationRunId`) REFERENCES `migration_runs`(`migrationRunId`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `certification_review_actions` ADD CONSTRAINT `cert_review_actions_certification_fk` FOREIGN KEY (`certificationId`) REFERENCES `certifications`(`id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE `certification_review_actions` ADD CONSTRAINT `cert_review_actions_staff_position_fk` FOREIGN KEY (`platformStaffPositionId`) REFERENCES `platform_staff_positions`(`id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `certification_review_actions` ADD CONSTRAINT `certification_review_actions_run_fk` FOREIGN KEY (`migrationRunId`) REFERENCES `migration_runs`(`migrationRunId`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
